...
public class BST<T extends Comparable<T>>{
    ...
    public POI search(double x, double y) {
        BinaryTreeNode<POI> node = (BinaryTreeNode<POI>) root;
        while (node != null) {
            POI poi = node.data;
            if (poi.getX() == x && poi.getY() == y) {
                return poi;
            }
            if (poi.getX() < x || (poi.getX() == x && poi.getY() < y)) {
                node = node.right;
            } else {
                node = node.left;
            }
    ...
    // search and return the list containing value
    // return null if no node found
    public void  search(String value,boundingRectangle rectangle)
    {
        int count = 0;
        inOrderTraversal(root, value, rectangle,count);
    }

    private void inOrderTraversal(BinaryTreeNode<T> node, String value, boundingRectangle rectangle,int count)
    {
        if (node == null || count == 50)
        {
            return;
        }

        inOrderTraversal(node.left, value, rectangle,count);

        POI tmp = (POI) node.data;

        if (tmp.getServices().contains(value) && rectangle.contains((POI)node.data))
        {
            count++;
            System.out.println(node.data);
        }

        inOrderTraversal(node.right, value,rectangle,count);
    }
