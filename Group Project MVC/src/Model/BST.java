...
public class BST<T extends Comparable<T>>
{
    ...
    private void inOrderTraversal(BinaryTreeNode<T> node, String value, boundingRectangle rectangle)
    {
        if (node == null )
        {
            return;
        ...
        if (count < 50)
        {
            inOrderTraversal(node.left, value, rectangle);
            ...
            if (tmp.getServices().contains(value) && rectangle.contains((POI) node.data))
            {
                ...
                {
                    count++;
                    System.out.printf("Location %d: %s%n",count,node.data);
                }
            }

            if (count < 50)
            {
                inOrderTraversal(node.right, value, rectangle);
            }
        }
    }
